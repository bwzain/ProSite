...
import { Headphones, BookOpen, Cpu, Compass, Terminal, CheckCircle2, Copy, Check, Shield } from "lucide-react";
import { PROFILE_DATA } from "@/data/profile";

export function AboutSection() {
  const [activeTab, setActiveTab] = useState<"story" | "terminal">("story");
  ...
  const terminalCode = `// WILLIAM ZAIN - ARCHITECT & CREATIVE PROFILE DIAGNOSTICS
{
  "name": "${PROFILE_DATA.name}",
  "location": "${PROFILE_DATA.location}",
  "status": "Enterprise IT Veteran // Digital Music Producer",
  "experience": {
    "enterprise_it": "30+ Years (Calance, Nintex, Isuzu)",
    "cloud_architecture": "AWS Certified Solutions Architect",
    "bpm_automation": "K2 Workflow, Nintex, Kryon RPA",
    "music_production": "Ableton Live, Zainy Beats",
    "authoring": "Introduction to Artificial Intelligence (Amazon)",
    "speaking": "Distinguished Toastmaster (DTM)"
  },
  "current_focus": [
    "Composing electronic beat soundscapes",
    "Demystifying AI for non-technical creators",
    "Publishing travel & cultural stories"
  ]
}`;
    ...
    <section className="py-20 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* Left: Tab Switcher & Main Story/Terminal */}
          <div className="lg:col-span-7 space-y-6">
            
            <div className="flex items-center gap-2 bg-slate-200 dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-300 dark:border-slate-800 w-fit">
              <button
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-5 py-2 rounded-xl text-xs font-bold transition-all ${
                  activeTab === "terminal"
                    ? "bg-slate-900 text-cyan-400 border border-cyan-500 shadow-md"
                    : "text-slate-700 dark:text-slate-300 hover:text-sky-600 dark:hover:text-sky-400"
                }`}
              >
            ...
            {activeTab === "story" ? (
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="rounded-3xl bg-slate-900 text-slate-100 border border-slate-800 overflow-hidden shadow-2xl font-mono text-xs"
              >
                <div className="flex items-center justify-between px-4 py-3 bg-slate-950 border-b border-slate-800">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-2 text-slate-400 text-[11px]">profile_diagnostics.json</span>
                  </div>
                  <button
                    onClick={copyCode}
                    className="flex items-center gap-1.5 text-slate-300 hover:text-white text-[11px] transition-all"
                  >
                    {copied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                    <span>{copied ? "Copied!" : "Copy JSON"}</span>
                  </button>
                </div>
                <div className="p-5 overflow-x-auto text-emerald-300 leading-relaxed">
